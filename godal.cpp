...
extern int
_gogdalMultiReadCallback(char *key, int nRanges, void *pocbuffers, void *coffsets, void *clengths, char **errorString);
...
godalNewCoordinateTransformation(cctx *ctx, OGRSpatialReferenceH src, OGRSpatialReferenceH dst) {
    godalWrap(ctx);
...
void godalContourGenerate(cctx *ctx, GDALRasterBandH hBand, double dfContourInterval, OGRLayerH layer, int idIdx, int elevIdx) {
    godalWrap(ctx);
    CPLErr ret = GDALContourGenerate(hBand, dfContourInterval, 0, 0, 0,
                                     false, 0, layer, idIdx, elevIdx, nullptr, nullptr);
    if (ret != 0) {
        forceCPLError(ctx, ret);
    }
    godalUnwrap();
...
void godalDatasetStructure(GDALDatasetH ds, int *sx, int *sy, int *bsx, int *bsy, double *scale, double *offset,
                           int *bandCount, int *dtype) {
...
void godalBandStructure(GDALRasterBandH bnd, int *sx, int *sy, int *bsx, int *bsy, double *scale, double *offset,
                        int *dtype) {
...
void
godalBandRasterIO(cctx *ctx, GDALRasterBandH bnd, GDALRWFlag rw, int nDSXOff, int nDSYOff, int nDSXSize, int nDSYSize,
                  void *pBuffer,
                  int nBXSize, int nBYSize, GDALDataType eBDataType, int nPixelSpace, int nLineSpace,
...
void
godalDatasetRasterIO(cctx *ctx, GDALDatasetH ds, GDALRWFlag rw, int nDSXOff, int nDSYOff, int nDSXSize, int nDSYSize,
                     void *pBuffer,
...
void
godalPolygonize(cctx *ctx, GDALRasterBandH in, GDALRasterBandH mask, OGRLayerH layer, int fieldIndex, char **opts) {
    godalWrap(ctx);
...
void godalSieveFilter(cctx *ctx, GDALRasterBandH bnd, GDALRasterBandH mask, GDALRasterBandH dst, int sizeThreshold,
                      int connectedNess) {
    godalWrap(ctx);
...
void
godalFillNoData(cctx *ctx, GDALRasterBandH in, GDALRasterBandH mask, int maxDistance, int iterations, char **opts) {
    godalWrap(ctx);
...
void godalRasterizeGeometry(cctx *ctx, GDALDatasetH ds, OGRGeometryH geom, int *bands, int nBands, double *vals,
                            int allTouched) {
...
void godalFeatureSetFieldDateTime(cctx *ctx, OGRFeatureH feat, int fieldIndex, int year, int month, int day, int hour,
...
void godalRasterHistogram(cctx *ctx, GDALRasterBandH bnd, double *min, double *max, int *buckets,
    ...
    if (*buckets == 0) {
        ...
        *values = (unsigned long long *) VSIMalloc(*buckets * sizeof(GUIntBig));
        ret = GDALGetRasterHistogramEx(bnd, *min, *max, *buckets, *values, bIncludeOutOfRange, bApproxOK, nullptr,
                                       nullptr);
    }
    if (ret != 0) {
        forceCPLError(ctx, ret);
...
void godalComputeRasterStatistics(cctx *ctx, GDALRasterBandH bnd, int bApproxOK, double *pdfMin, double *pdfMax,
...
int
godalGetRasterStatistics(cctx *ctx, GDALRasterBandH bnd, int bApproxOK, double *pdfMin, double *pdfMax, double *pdfMean,
                         double *pdfStdDev) {
    godalWrap(ctx);
    CPLErr ret = CE_None;
...
void
godalSetRasterStatistics(cctx *ctx, GDALRasterBandH bnd, double dfMin, double dfMax, double dfMean, double dfStdDev) {
    godalWrap(ctx);
    CPLErr ret = CE_None;
...
namespace cpl {
    ...
    int
    VSIGoHandle::ReadMultiRange(int nRanges, void **ppData, const vsi_l_offset *panOffsets, const size_t *panSizes) {
        int iRange;
        int nMergedRanges = 1;
        ...
        if (nMergedRanges == nRanges) {
            int ret = _gogdalMultiReadCallback(m_filename, nRanges, (void *) ppData, (void *) panOffsets,
                                               (void *) panSizes, &err);
            if (err) {
                CPLError(CE_Failure, CPLE_AppDefined, "%s", err);
                errno = EIO;
        ...
        int ret = _gogdalMultiReadCallback(m_filename, nRanges, (void *) ppData, (void *) panOffsets, (void *) panSizes,
                                           &err);
